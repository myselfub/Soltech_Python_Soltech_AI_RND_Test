...
class Pipeline:
    ...
    """
    def __init__(self):
        self.name: str = 'API RAG Pipeline'
    ...
    def pipe(
        ...
        # _message: str = messages[-1].get('content')
        #_message: str = '태그명 목록 알려줘'
        _message: str = '사무실(3F) 3월 평균 온도랑 최고온도 알려줘.'
        ...
        try:
            ...
            logger.info('------------------------')
            logger.info(_data_response_dict)
            logger.info('------------------------\n')
            ...
            _api_llm_response: str = _llm.predict(_api_template)
            # for _chunk in _llm.stream(_api_template):
            #    yield _chunk

            logger.info(_api_llm_response)
            return _api_llm_response
