...
author: Soltech
version: 1.0
requirements: llama-index-embeddings-ollama
"""
...
from test.agent import fc_api_test, fc_db_test
...
class Pipeline:
    ...
    llm: Ollama = None
    db_tools: fc_db_test.DBAgent = None
    api_tools: fc_api_test.APIAgent = None
    ...
    def on_startup(self) -> None:
        ...
        self.db_tools: fc_db_test.DBAgent = self._init_db_agent_tools()
        self.api_tools: fc_api_test.APIAgent = self._init_api_agent_tools()
    ...
    """
        DB Agent 초기화
    """
    def _init_db_agent_tools(self) -> fc_db_test.DBAgent:
        return fc_db_test.DBAgent(pipeline_valves=self.valves, llm=self.llm, embed_model=self.embed_model)

    """
        API Agent 초기화
    """
    def _init_api_agent_tools(self) -> fc_api_test.APIAgent:
        return fc_api_test.APIAgent(pipeline_valves=self.valves, llm=self.llm, embed_model=self.embed_model)
    ...
    def pipe(
        ...
        _chat_history = self._parse_chat_history(user_message=user_message, messages=messages)
...
response_gen = pipeline.pipe(msg_01, '', history, {})
res_01 = ''
...
response_gen = pipeline.pipe(msg_02, '', history, {})
res_02 = ''
for text in response_gen:
    res_02 += text
    print(text, end='', flush=True)
...
response_gen = pipeline.pipe(msg_03, '', history, {})
res_03 = ''
for text in response_gen:
    res_03 += text
    print(text, end='', flush=True)
...
msg_04 = '휴게실 온도'
history.append({'role': 'assistant', 'content': str(res_03)})
history.append({'role': 'user', 'content': msg_04})
response_gen = pipeline.pipe(msg_04, '', history, {})
for text in response_gen:
    print(text, end='', flush=True)
