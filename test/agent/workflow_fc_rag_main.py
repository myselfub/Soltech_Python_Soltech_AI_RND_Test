...
from test.agent import workflow_fc_db_test, workflow_fc_api_test
...
class Pipeline:
    ...
    llm: Ollama = None
    db_tools: workflow_fc_db_test.DBAgent = None
    api_tools: workflow_fc_api_test.APIAgent = None
    ...
    def on_startup(self) -> None:
        ...
        self.db_tools: workflow_fc_db_test.DBAgent = self._init_db_agent_tools()
        self.api_tools: workflow_fc_api_test.APIAgent = self._init_api_agent_tools()
    ...
    def _init_db_agent_tools(self) -> workflow_fc_db_test.DBAgent:
        """
            DB Agent 초기화
        """
        return workflow_fc_db_test.DBAgent(pipeline_valves=self.valves, llm=self.llm, embed_model=self.embed_model)

    def _init_api_agent_tools(self) -> workflow_fc_api_test.APIAgent:
        """
            API Agent 초기화
        """
        return workflow_fc_api_test.APIAgent(pipeline_valves=self.valves, llm=self.llm, embed_model=self.embed_model)
